...
class Demo:
    ...
    def run(self) -> None:
        while True:
            ...
            if DEMO:
                # pts = draw_utils.display_canv(CANV_MODE=CANV_MODE, cur_pos=mid_point)  # cur_pos=cur_pos   # 수정 해야 할 부분
                pts = draw_utils.display_easyocr_canv(CANV_MODE=CANV_MODE, cur_pos=mid_point)
                self.pts.append(pts)
...
def main():
    ...
    if CUST_VIDEO:
        ...
        print('CEP95 = ', CE95)
        # draw results
        draw_utils.plot_pts((demo.true_pos, demo.cur_pos), str_name, MAE, save_path)  # 여기에서 시선 벡터의 시각적 표시하는 부분을 의미함. # 수정해야할 부분
